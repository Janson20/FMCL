...
# 动态 User-Agent，基于 updater.get_current_version()
try:
    from updater import get_current_version
    FMCL_USER_AGENT = f"FMCL/{get_current_version()}"
except Exception:
    FMCL_USER_AGENT = "FMCL/unknown"
...
class AsyncBatchDownloader:
    ...
    def _sync_fallback(
        ...
        for i, (url, save_path) in enumerate(tasks):
            try:
                Path(save_path).parent.mkdir(parents=True, exist_ok=True)
                resp = requests.get(url, timeout=30, headers={"User-Agent": FMCL_USER_AGENT})
