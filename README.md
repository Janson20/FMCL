...
| 🎵 音乐播放器 | 本地播放 + 在线搜索（酷我/酷狗/咪咕/QQ/网易） | 双模式切换、歌词、桌面歌词、EQ/混响/变调音效、SMTC、搜索结果原唱置顶标签与争议老歌策展、原唱错标/漏标一键反馈问卷 |
...
- **Windows**: 双击 `.exe` 安装包，按向导完成安装。安装包已内置 7-Zip（24.09 版本），无需联网即可自动安装。安装器按当前用户安装到 `%LOCALAPPDATA%\Programs\FMCL`，**全程无需管理员权限**，安装后直接双击快捷方式即可正常使用
