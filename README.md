...
| 🎵 音乐播放器 | 本地播放 + 在线搜索（酷我/酷狗/咪咕/QQ/网易） | 双模式切换、歌词、桌面歌词、EQ/混响/变调音效、SMTC、搜索结果原唱置顶标签（规则化识别 + 平台原唱歌曲 ID 策展：命中 ID 的版本全部置顶并标原唱，支持同一歌名多原唱如《脑壳疼》洛天依、ilem 与 洛天依Official、ilem 双版；平台无音源歌曲以徽章显示真实原唱名；同名不同曲不标记）、原唱错标/漏标一键反馈问卷、VIP/无版权歌曲自动跨音源兜底播放（含哔哩哔哩，兜底自动选择最高可用音质，状态栏提示切换结果）、自动音质选择（按账号可用最高音质）与当前播放音质标签（本地按文件码率）、设置内扫码登录网易云音乐账号后直接播放 VIP 歌曲并获取 VIP 歌词（显示会员等级与对应音质） |
