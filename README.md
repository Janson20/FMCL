...
| 🎵 音乐播放器 | 本地播放 + 在线搜索（酷我/酷狗/咪咕/QQ/网易） | 双模式切换、歌词、桌面歌词、EQ/混响/变调音效、SMTC、搜索结果原唱置顶标签与争议老歌策展（含原唱日期为平台占位日期/缺失的修正、同名不同曲不标记）、原唱错标/漏标一键反馈问卷、VIP/无版权歌曲自动跨音源兜底播放（含哔哩哔哩，兜底自动选择最高可用音质，状态栏提示切换结果）、自动音质选择（按账号可用最高音质）与当前播放音质标签（本地按文件码率）、设置内扫码登录网易云音乐账号后直接播放 VIP 歌曲并获取 VIP 歌词（显示会员等级与对应音质） |
