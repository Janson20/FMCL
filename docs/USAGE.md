...
1. 在中间列表点击要配置的服务器，再点右下角「🔧 服务器配置」；或直接点该服务器一行右侧的 🔧 按钮
2. 窗口左侧按用途分成 7 类：🌍 世界与游戏 / 👥 玩家与规则 / 🌐 网络与连接 / ⚡ 性能优化 / 🎨 资源包 / 🚀 启动设置 / ✏️ 原始文件
3. 每个配置项都显示「中文名称 + 原始键名 + 一句话说明」，需要填数字的还会显示取值范围；「高级」标记的项一般不用改
4. 顶部搜索框支持按名称、键名（如 `motd`）或说明文字模糊搜索
5. 右上角实时显示「已修改 N 项」，改完点「💾 保存配置」写回 `server.properties`

> 💡 保存只会更新改动过的键，文件里的注释和其他键（例如整合包自带的配置）都会保留。改坏了可以点「↺ 重新加载」放弃修改，或点「♻ 恢复默认」把当前分类恢复成新装服务器的默认值。
> 💡 服务器正在运行时保存不会立即生效，需要重启服务器。
> 💡 「🚀 启动设置」里可以为这个服务器单独设置最大内存，只对当前服务器生效；开服页面右侧的内存下拉框也是针对当前选中服务器的。
> 💡 需要直接编辑文本时，切到「✏️ 原始文件」分类即可看到 `server.properties` 原文，点「📝 写入文件」保存。
...
- 本机创建的陶瓦房间与 HMCL（Terracotta）/ PCL CE 的房间完全互通，双方可互相加入，大厅编号（`U/` 前缀）格式一致
- 共用 EasyTier 官方内核（与 PCL CE 同款版本），与 HMCL / FCL 的陶瓦定制版协议互通；若首次使用本页需点击「安装 EasyTier」，安装时自动测速选择最快镜像
- 加入他人房间时会自动校验房间信令服务（Scaffolding 指纹握手），失败时日志面板会给出明确原因
...
| 工具 | 说明 |
|------|------|
| 清理游戏垃圾 | 扫描并删除自定义文件夹（默认 `.minecraft`）下的 `.log` 和 `.tmp` 文件，支持自定义扫描深度（默认 5 层），结果按目录折叠展示（可展开勾选单个文件），目录按总大小从大到小排列 |
