...
- **跨音源兜底**：当前音源无法获取播放链接（VIP/无版权等）或下载文件无效（HTML 错误页/试听片段）时，自动在其它音源搜索同款歌（歌名+歌手，时长 ±15 秒校验排除翻唱/伴奏）并替换播放，并在左下角状态栏提示切换结果；兜底源除酷我/酷狗/咪咕/QQ/网易外还包含哔哩哔哩（dash 纯音频流，WBI 签名，触发交互式风控时自动跳过），类似 YesPlayMusic 的 UnblockNeteaseMusic 思路——不破解，换平台找同曲
