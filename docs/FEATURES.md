...
- **原唱识别与置顶（网易云音源）**：搜索结果中自动识别原唱并置顶标「原唱」标签，核心算法：
  ...
  - **原唱 ID 策展表**（`CURATED_ORIGINAL_IDS`，约 60 个平台原唱歌曲 ID）：平台官方 ID 级判定，命中即全部置顶标原唱（支持同一歌名多原唱，如《脑壳疼》洛天依、ilem 与 洛天依Official、ilem 双版），也能标记原唱不在歌名搜索结果页内的歌曲（如《再回首》苏芮版、《未来へ》Kiroro）
  - **同名不同曲表**（如《蝴蝶》《彼岸花》《超能力》等）：无法判定用户搜索意图，整组不标记任何原唱
  - **翻唱歌手规则**：结果中同时出现歌手含"封茗囧菌/洛少爷"的歌曲与歌手含"洛天依/洛天依Official"的同名歌曲时，只标洛天依版本为原唱——有"洛天依Official"只标全部 Official 版本，否则标歌手里含"洛天依"且发表最早的一个（这两名歌手以翻唱洛天依曲目为主；两首须为不同歌曲，歌手集合互斥防合唱误判）
  - **无音源原唱表**（`NO_SOURCE_ORIGINALS`，如《月亮代表我的心》陈芬兰）：平台无原唱音源时置顶最热版本，由徽章显示真实原唱名
